...
class Ko_General extends CI_Model {
  ...
  public function get_data($column = '*')
  {
    ...
    $this->db->join('ko_general_status e', 'a.id = e.id_ko AND a.status = e.status');
  ...
  public function show($id, $column = '*')
  {
    ...
    $this->db->join('ko_general_status d', 'a.id = d.id_ko AND a.status = d.status');
  ...
  public function show_detail($id, $column = '*')
  {
    $this->db->select($column, FALSE);
    $this->db->from('ko_general a');
    $this->db->join('ko_general_detail b', 'a.id = b.id_ko');
  ...
  public function show_onoff($id, $column = '*')
  {
    $this->db->select($column, FALSE);
    $this->db->from('ko_general a');
    $this->db->join('ko_general_onoff b', 'a.id = b.id_ko');
    $this->db->where('a.id', $id);
    $this->db->where('a.tahun', $this->session->userdata('tahun'));
    $this->db->where('a.hapus', null);
    $result = $this->db->get();
  ...
  public function show_onoff_total($id, $column = '*')
  {
    $this->db->select($column, FALSE);
    $this->db->from('ko_general a');
    $this->db->join('ko_general_onoff_total b', 'a.id = b.id_ko');
    $this->db->where('a.id', $id);
    $this->db->where('a.tahun', $this->session->userdata('tahun'));
    $this->db->where('a.hapus', null);
    $result = $this->db->get();
